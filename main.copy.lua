...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
    ...
    elseif effect == 1 then
        -- keep hp cap for characters like keeper
        ...
        if heartTotal < p:GetHeartLimit() then
            -- logical ballache for the support of the Forgotten
            -- fuck subplayers man
            local remainder = math.floor((p:GetHeartLimit() - heartTotal)/2)
            local amountToAdd = math.floor(2/2)
            print(remainder)
            print(amountToAdd)
            if remainder < amountToAdd then amountToAdd = remainder end
            print(amountToAdd)
            if p:GetPlayerType() == PlayerType.PLAYER_THESOUL then
                if not data.whichSub then
                    data.whichSub = "soul"
                elseif data.whichSub == "forgotten" then
                    data.whichSub = "both"
                end
                if not data.soulHp then data.soulHp = 0 end
                data.soulHp = data.soulHp + amountToAdd
            elseif p:GetPlayerType() == PlayerType.PLAYER_THEFORGOTTEN then
                if not data.whichSub then
                    data.whichSub = "forgotten"
                elseif data.whichSub == "soul" then
                    data.whichSub = "both"
                end
                if not data.forgorHp then data.forgorHp = 0 end
                data.forgorHp = data.forgorHp + amountToAdd
            else
...
mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, function()
    ...
    for i=0,game:GetNumPlayers()-1 do
        ...
        if data.redHp then
            for i=1,data.redHp do
                p:AddMaxHearts(-2)
            end
            data.redHp = 0
        end
        if data.soulHp then
            for i=1,data.soulHp do
                if (p:GetPlayerType() == PlayerType.PLAYER_THEFORGOTTEN and (data.whichSub == "soul" or data.whichSub == "both")) then
                    p:GetSubPlayer():AddMaxHearts(-2)
                elseif (p:GetPlayerType() == PlayerType.PLAYER_THESOUL) then
                    p:AddMaxHearts(-2)
                end
            end
            data.soulHp = 0
        end
        if data.forgorHp then
            for i=1,data.forgorHp do
                if (p:GetPlayerType() == PlayerType.PLAYER_THESOUL and (data.whichSub == "forgotten" or data.whichSub == "both")) then
                    p:GetSubPlayer():AddMaxHearts(-2)
                elseif (p:GetPlayerType() == PlayerType.PLAYER_THEFORGOTTEN) then
                    p:AddMaxHearts(-2)
                end
        ...
        if data.whichSub then data.whichSub = nil end
        ...
        if data.extraMagneto then
            for i=1,data.extraMagneto do
                p:RemoveCollectible(CollectibleType.COLLECTIBLE_MAGNETO)
            end
            data.extraMagneto = nil
        end
    end
...
SimpleLootCardItem(k.curvedHorn, i.curvedHorn)
mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, function()
    for x=0,game:GetNumPlayers()-1 do
        local p = Isaac.GetPlayer(x)
        local data = p:GetData()
...
SimpleLootCardItem(k.purpleHeart, i.purpleHeart)
mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, function()
    local room = game:GetRoom()
    for x=0,game:GetNumPlayers()-1 do
        local p = Isaac.GetPlayer(x)
...
SimpleLootCardItem(k.goldenHorseshoe, i.goldenHorseshoe)
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
...
SimpleLootCardItem(k.guppysHairball, i.guppysHairball)
mod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, e)
    local p = e:ToPlayer()
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
    ...
    if heartTotal < p:GetHeartLimit() then
        ...
        -- fuck subplayers man
        local remainder = math.floor((p:GetHeartLimit() - heartTotal)/2)
        local amountToAdd = math.floor(4/2)
        print(remainder)
        print(amountToAdd)
        if remainder < amountToAdd then amountToAdd = remainder end
        print(amountToAdd)
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
    ...
    local spawnedItem = Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, collectible, room:FindFreePickupSpawnPosition(p.Position), Vector.Zero, nil):ToPickup()
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
    ...
    elseif effect == 5 then
        ...
        local spawnedItem = Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, collectible, room:FindFreePickupSpawnPosition(p.Position), Vector.Zero, nil)
    end
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
    ...
    else data.strength = data.strength + 1 end
    p:AddCacheFlags(CacheFlag.CACHE_DAMAGE)
    p:EvaluateItems()
end, k.strength)
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
    ...
    if not data.extraMagneto then data.extraMagneto = 1
    else data.extraMagneto = data.extraMagneto + 1 end
end, k.theHangedMan)

mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
    local data = p:GetData()
    if not data.deathRevive then data.deathRevive = true end
    p:Die()
end, k.death)

-- TODO: Follow tweaks done below to prevent save/continue/streaking bugs, add visual/audio indicators, stacking increases revive to 50%
-- fix death animation end to support "lostdeath" (its shorter)
-- keeper should revive with one coin heart and the flies of 3 bone hearts
mod:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, function(_, p)
    --[[ TODO:
    a good algorithm for revive is to:
    1) prevent fatal damage in EntityTakeDmg callback
    2) give curse of unknown, if it doesn't already exist
    3) play death animation
    4) at the end of the animation, manually move player to previous room
    5) play animation for holding revival item
    6) remove curse of unknown, if necessary    ]]
    local sprite = p:GetSprite()
    local data = p:GetData()
    level = game:GetLevel()
    room = level:GetCurrentRoom()
    if ( sprite:IsPlaying("Death") or sprite:IsPlaying("LostDeath") ) and sprite:GetFrame() >= 50 then
        if data.deathRevive then
            p:Revive()
            local enterDoor = level.EnterDoor
            local door = room:GetDoor(enterDoor)
            local direction = door and door.Direction or Direction.NO_DIRECTION
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
    local itemPool = game:GetItemPool()
    local collectible = itemPool:GetCollectible(itemPool:GetPoolForRoom(RoomType.ROOM_TREASURE, rng:GetSeed()))
...
mod:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, function(_, p)
    ...
    if data.moon == 1 then
        ...
        if data.moonTimer <= 0 then
            Isaac.Spawn(EntityType.ENTITY_SHOPKEEPER, 0, 0, room:GetRandomPosition(0), Vector.Zero, nil)
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, c, p)
    ...
    for i=0,rng:RandomInt(3) do
        Isaac.Spawn(EntityType.ENTITY_PICKUP, reward[1], reward[2], p.Position, Vector.FromAngle(rng:RandomInt(360)), nil)
    end
end, k.judgement)
