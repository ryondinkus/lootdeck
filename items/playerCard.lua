...
local function MC_POST_PEFFECT_UPDATE(_, p)
    ...
    if data[Tag] then
        if p:IsExtraAnimationFinished() then
            data[Tag] = nil
            helper.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_TAROTCARD, helper.GetWeightedLootCardId(true), Game():GetRoom():FindFreePickupSpawnPosition(p.Position), Vector.Zero, nil)
        end
    else
